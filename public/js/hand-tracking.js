document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayARScene(arExperience) {
        ...
        switch (arExperience.outputType) {
            case 'image':
                outputEntity = `<a-image src="${arExperience.output}" width="0.1" height="0.1"></a-image>`;
                break;
            case 'video':
                outputEntity = `<a-video src="${arExperience.output}" width="0.1" height="0.1" autoplay loop></a-video>`;
                break;
            case 'youtube':
                outputEntity = `<a-entity geometry="primitive: plane; width: 0.1; height: 0.05625" material="shader: flat; src: #yt-video"></a-entity>
                                <a-assets>
                                    <video id="yt-video" src="${arExperience.output}" autoplay loop crossorigin="anonymous"></video>
                                </a-assets>`;
                break;
            case '3d':
                outputEntity = `<a-entity gltf-model="${arExperience.output}" scale="0.05 0.05 0.05"></a-entity>`;
                break;
        }

        arScene.innerHTML = `
            <a-scene mindar-hand embedded>
                <a-camera active="false" position="0 0 0"></a-camera>
                <a-entity mindar-hand-target="anchorIndex: 1">
                    ${outputEntity}
                </a-entity>
            </a-scene>
        `;
